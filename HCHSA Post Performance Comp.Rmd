...
```{r message=FALSE, warning=FALSE, CACHE = TRUE}
#  Identifying top 5 most performing post from December 2022 to December 2023
p <- ggplot(top_posts, aes(x = Rank, y = Engagements, fill = Network)) +
    geom_bar(stat = "identity") +
    geom_text(aes(label = Link, y = Engagements / 2), 
              angle = 90, 
              hjust = 0.5, 
              vjust = 0.5, 
              color = "black", 
              size = 3) +
    labs(title = 'Top 5 Performing Posts Dec 22-Dec 23',
         x = 'Rank',
         y = 'Engagements') +
    theme_minimal() +
...
ggsave("top_posts_chart.png", plot = p, width = 12, height = 8, dpi = 300)
```